...
class IMManager {
  ...
  Future<void> init({
    ...
    try {
      ...
      // 群组监听
      _manager.setGroupListener(listener: _groupListener);
  ...
  Future<bool> login({required String userID, required String userSig}) async {
    var result = await _manager.login(userID: userID, userSig: userSig);
  ...
  /// 设置消息已读
  Future<void> setMessageAsRead({String? uid, String? gid}) async {
    try {
  ...
  /// 重新发送消息
  /// [msgID] 消息 msgID
  Future<V2TimMessage> reSendMessage({required String msgID}) async {
    try {
  ...
  /// 删除本地消息
  /// [msgID] 消息 msgID
  Future<int> deleteMessageFromLocalStorage({required String msgID}) async {
    try {
      final result = await _manager.v2TIMMessageManager
  ...
  /// 删除本地及漫游消息
  /// [msgID] 消息 msgID
  Future<int> deleteMessages({required String msgID}) async {
    try {
